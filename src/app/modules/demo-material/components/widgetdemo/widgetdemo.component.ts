...
import { AcTabData } from '../../../demo-ui/components/tabs/tabs/models/actabdata.model';
import { AcTab } from '../../../demo-ui/components/tabs/tabs/models/actab.model';
...
export class WidgetDemoComponent {
  ...
  constructor(private router: Router, private activatedRoute: ActivatedRoute) {
    ...
    this.activatedRoute.url.subscribe(url => {
      const userRoutes: AcTab[] = new Array<AcTab>();
      const current = url[url.length - 1].path;
      router.config
      .filter((route) => route.path === 'mat')
      .shift().children
      ...
      .forEach(subroute =>
        ...
        userRoutes.push(new AcTab(['/mat/widget', current, subroute.path], subroute.data.title, !!subroute.data.active, !!subroute.data.disabled))
      );
